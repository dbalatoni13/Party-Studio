...
namespace CafeLibrary.Rendering
{
    ...
    public class BfshaRenderer : ShaderRenderBase
    {
        ...
        public void LoadVertexShaderConstantBlock(int programID)
        {
            ...
            firstBlock.SetData(GLShaderInfo.VertexConstants);
            firstBlock.Render(programID, "vp_c1", 0);
        }
        ...
        public void LoadPixelShaderConstantBlock(int programID)
        {
            ...
            firstBlock.SetData(GLShaderInfo.PixelConstants);
            firstBlock.Render(programID, "fp_c1", 1);
        }
        ...
        public virtual void SetBoneMatrixBlock(STSkeleton skeleton, Matrix4 root, bool useInverse, UniformBlock block, int maxTransforms = 170)
        {
            ...
            for (int i = 0; i < maxTransforms; i++)
            {
                ...
                block.BufferCreator.Add(value.Column0);
                block.BufferCreator.Add(value.Column1);
                block.BufferCreator.Add(value.Column2);
            }
        }
        ...
        public void SetTextureUniforms(GLContext control, ShaderProgram shader, STGenericMaterial mat)
        {
            ...
            for (int i = 0; i < ShaderModel.Samplers.Count; i++)
            {
                ...
                {
                    var texture = ScreenBufferTexture.GetColorBuffer();
        ...
        {
            if (vertexLocation != -1)
                block.Render(programID, IsSwitch ? $"vp_c{vertexLocation + 3}" : $"vp_{vertexLocation}", binding);

            if (fragmentLocation != -1)
                block.Render(programID, IsSwitch ? $"fp_c{fragmentLocation + 3}" : $"fp_{fragmentLocation}", binding);
        }
        ...
        private UniformBlock GetBlock(string name, bool reset = true)
        {
            ...
            if (reset)
                UniformBlocks[name].BufferCreator.Clear();
