...
namespace PartyStudio
{
    public class MainWindow : UIFramework.MainWindow
    {
        ...
        private void Init()
        {
            ...
            fileMenu.MenuItems.Add(new MenuItem($"Open Board", IconManager.OPEN_ICON, () =>
            {
                BoardLoaderUI boardLoaderUI = new BoardLoaderUI();
                DialogHandler.Show("Board Loader", boardLoaderUI.Render, (o) =>
                {
                    if (o)
                    {
                        boardLoaderUI.LoadBoard();
                        LoadFileFormat(boardLoaderUI.FilePath);
                    }
                }, 400, 300);
            }));
          /*  fileMenu.MenuItems.Add(new MenuItem($"Open Player Viewer", IconManager.OPEN_ICON, () =>
            {
                ...
                DialogHandler.Show("Player Viewer", playerViewer.LoadGUI, (o) =>
                {
                    if (o)
                    {
                        ...
                        playerViewer.Load();
                        workspace.ActiveEditor = playerViewer;
                        workspace.SetupActiveEditor(playerViewer);

                        workspace.ViewportWindow.Pipeline.AddFile(playerViewer, "Player Viewer");
                    }
                }, 400, 300);
            }));*/
        ...
        private void CreatePlayerViewer()
        {
            CreateNewProject(typeof(PartyStudio.GCN.MPBIN));
        }
        ...
        private void LoadFileFormat(string filePath)
        {
            ...
            if (!isProject)
            {
                bool createNew = !Outliner.AddToActiveWorkspace;
